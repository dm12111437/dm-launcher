...
- **图标（直接用 exe 默认图标）**：Electron 原生 `app.getFileIcon` 进程内获取 exe 的 Windows 图标；自动检测**通用蓝白图标**（像素颜色数过少）并换用同文件夹其他 exe 的真实图标（如 VALORANT.exe 无图标 → 用登录器的图标）；缓存复用、永不清理；设置里「🖼 重新识别图标」可重试失败项
